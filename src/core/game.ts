// 游戏逻辑
import { useGlobalStore } from "./gameStore";
// @ts-ignore
...
const useGame = () => {
  ...
  //游戏状态：0 - 初始化, 1 - 进行中, 2 - 失败结束, 3 - 胜利，用于显示选块区及胜利模块等
  ...
  // 选块区总共划分 24 x 24 的格子，每个块占 3 x 3 的格子，生成的起始 x 和 y 坐标范围均为 0 ~ 21
   ...
   */
  const initGame = () => {
    console.log("game initing...", gameConfig);
    ...
    // 1. 规划块数
    // 块数单位（总块数必须是该值的倍数）即运动类别数*（需要多少个才能消除）
    ...
    // 需要用到的动物数组
    const needSports = gameConfig.sports.slice(0, gameConfig.typeNum);
    // 依次把格子塞到数组里
    for (let i = 0; i < totalBlockNum.value; i++) {
      sportBlocks.push(needSports[i % gameConfig.typeNum]);
    }
    // 打乱数组
    const randomSportBlocks = _.shuffle(sportBlocks);
    ...
    let minY = 0;
    let maxY = 22;
    // 分为 gameConfig.levelNum（选块区块的层数）层，依次生成，每层的边界不同
    ...
    console.log("最终剩余格子数", leftBlockNum);
  ...
  /**
   * 给快绑定层级关系（用于确定哪些格子是可点击的）
   * 核心思想：每个块压住和其坐标有交集棋盘格内所有 level 大于它的点，双向建立联系
   */
   ...
   * 点击块事件
   */
  //randomIdx 随机区域下标，>= 0 表示点击的是随机块
  // force 强制移除
  const doClickBlock = (block: BlockType, randomIdx = -1, force = false) => {
    // 判断是否符合：已经输了 / 已经被点击 / 有上层块（且非强制和圣光）三种情况，不能再点击
    if (
    ...
    if (randomIdx >= 0) {
      ...
      //移除覆盖关系：将被该块压住的块的被压住状态id去除
      //_.remove:移除数组中调用函数（断言）返回为真值的所有元素，返回移除元素组成的数组
